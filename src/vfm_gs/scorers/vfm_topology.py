...
def _compute_pixel_error_map(rendered_image, viewpoint_cam, backend):
    ...
    raise ValueError(
        "Unsupported vfm_backend {!r}. Available mock backends: mock_l1, mock_edge_l1.".format(backend)
    )
...
def compute_gaussian_score_fastgs_with_vfm(camlist, gaussians, pipe, bg, args, DENSIFY=False):
    ...
    for viewpoint_cam in camlist:
        ...
        pixel_error_map = _compute_pixel_error_map(rendered_image, viewpoint_cam, backend)
