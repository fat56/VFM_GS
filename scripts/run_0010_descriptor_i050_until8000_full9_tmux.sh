...
SESSION_G0="0010_i050_u8000_full9_g0"
SESSION_G1="0010_i050_u8000_full9_g1"
SESSION_MERGE="0010_i050_u8000_full9_merge"
RUN_NAME="descriptor_i050_until8000_30k_r_auto"
METHOD="descriptor_i050_until8000"
EXTRA_OUT="output/0010/descriptor_i050_until8000_full9_extra"
PILOT_OUT="output/0010/descriptor_i050_until8000_pilot"
COMBINED_OUT="output/0010/descriptor_i050_until8000_full9_combined"
CONFIG="configs/experiments/0010_descriptor_i050_active_until8000.yaml"

usage() {
  cat <<'EOF'
Usage:
  scripts/run_0010_descriptor_i050_until8000_full9_tmux.sh start
  scripts/run_0010_descriptor_i050_until8000_full9_tmux.sh worker-g0
  scripts/run_0010_descriptor_i050_until8000_full9_tmux.sh worker-g1
  scripts/run_0010_descriptor_i050_until8000_full9_tmux.sh merge
...
existing_pids_for() {
  ...
  ps -eo pid=,cmd= \
    | awk -v group="$group" '
      ...
      $0 ~ "output/0010/descriptor_i050_until8000_full9_extra/" group &&
      $0 !~ "awk" { print $1 }
    ' \
    | tr '\n' ' '
}
...
wait_for_pids() {
  ...
  echo "[0010-u8000-full9] waiting for existing foreground PIDs: $pids"
  for pid in $pids; do
...
archive_incomplete() {
  ...
  if [[ -d "$run_dir" && ! -s "$result_file" ]]; then
    ...
    archive_root="output/0010/debug_artifacts/interrupted_until8000_full9_${stamp}/$(basename "$out_root")/$scene"
    ...
    echo "[0010-u8000-full9] archived incomplete $scene to $archive_root"
  fi
}
...
run_worker() {
  ...
  if [[ "${#missing[@]}" -eq 0 ]]; then
    echo "[0010-u8000-full9] no missing scenes for $out_root"
    return
  fi

  echo "[0010-u8000-full9] GPU $gpu running scenes: ${missing[*]}"
...
run_name = "descriptor_i050_until8000_30k_r_auto"
method = "descriptor_i050_until8000"
pilot_out = Path("output/0010/descriptor_i050_until8000_pilot")
extra_out = Path("output/0010/descriptor_i050_until8000_full9_extra")
combined = Path("output/0010/descriptor_i050_until8000_full9_combined")
...
scene_source = {
    "bicycle": (pilot_out, "mip_g0"),
    "garden": (pilot_out, "mip_g0"),
    "stump": (pilot_out, "mip_g0"),
    "counter": (pilot_out, "mip_g1"),
    "kitchen": (pilot_out, "mip_g1"),
    "bonsai": (pilot_out, "mip_g1"),
    "flowers": (extra_out, "mip_g0"),
    "treehill": (extra_out, "mip_g0"),
    "room": (extra_out, "mip_g1"),
}
...
missing = []
for scene in scenes:
    root, group = scene_source[scene]
    run_dir = root / group / scene / run_name
    result_path = run_dir / "results.json"
    if not result_path.exists():
        missing.append(scene)
        continue
    metrics = read_results(run_dir)
    gs, train_time = parse_train_log(root / group / scene / "logs" / run_name / "train.log")
    rows.append({
        "dataset": "mipnerf360",
        "scene": scene,
        "method": method,
...
summary = {
    ...
    "avg_psnr": avg([row["psnr"] for row in rows]),
    "avg_ssim": avg([row["ssim"] for row in rows]),
    "avg_lpips": avg([row["lpips"] for row in rows]),
    "avg_gs_num": avg([row["gs_num"] for row in rows]),
    "avg_train_time_s": avg([row["train_time_s"] for row in rows]),
    "avg_delta_psnr_phase0": avg([row["delta_psnr"] for row in phase0_comp]),
    "avg_delta_ssim_phase0": avg([row["delta_ssim"] for row in phase0_comp]),
    "avg_delta_lpips_phase0": avg([row["delta_lpips"] for row in phase0_comp]),
    "avg_delta_gs_num_phase0": avg([row["delta_gs_num"] for row in phase0_comp]),
    "avg_qcgi_phase0": avg([row["qcgi"] for row in phase0_comp]),
    "avg_delta_psnr_i050_full": avg([row["delta_psnr"] for row in full_comp]),
    "avg_delta_ssim_i050_full": avg([row["delta_ssim"] for row in full_comp]),
    "avg_delta_lpips_i050_full": avg([row["delta_lpips"] for row in full_comp]),
    "avg_delta_gs_num_i050_full": avg([row["delta_gs_num"] for row in full_comp]),
    "avg_qcgi_i050_full": avg([row["qcgi"] for row in full_comp]),
}
...
start_sessions() {
  ...
  if ! tmux has-session -t "$SESSION_G0" 2>/dev/null; then
    tmux new-session -d -s "$SESSION_G0" \
      "cd '$ROOT_DIR' && export WAIT_PIDS='$wait_g0' && bash scripts/run_0010_descriptor_i050_until8000_full9_tmux.sh worker-g0 2>&1 | tee output/0010/debug_logs/${SESSION_G0}.log"
  fi
  if ! tmux has-session -t "$SESSION_G1" 2>/dev/null; then
    tmux new-session -d -s "$SESSION_G1" \
      "cd '$ROOT_DIR' && export WAIT_PIDS='$wait_g1' && bash scripts/run_0010_descriptor_i050_until8000_full9_tmux.sh worker-g1 2>&1 | tee output/0010/debug_logs/${SESSION_G1}.log"
  fi
  if ! tmux has-session -t "$SESSION_MERGE" 2>/dev/null; then
    tmux new-session -d -s "$SESSION_MERGE" \
      "cd '$ROOT_DIR' && while tmux has-session -t '$SESSION_G0' 2>/dev/null || tmux has-session -t '$SESSION_G1' 2>/dev/null; do sleep 60; done; bash scripts/run_0010_descriptor_i050_until8000_full9_tmux.sh merge 2>&1 | tee output/0010/debug_logs/${SESSION_MERGE}.log"
  fi

  tmux ls | grep '0010_i050_u8000_full9' || true
}
...
case "${1:-}" in
  start)
    ...
    ;;
  worker-g0)
    run_worker 0 "$EXTRA_OUT/mip_g0" flowers treehill
    ;;
  worker-g1)
    run_worker 1 "$EXTRA_OUT/mip_g1" room
    ;;
  merge)
    merge_results
    ;;
