...
SESSION_G0="0011_i050_u4000_g0"
SESSION_G1="0011_i050_u4000_g1"
SESSION_MERGE="0011_i050_u4000_merge"
RUN_NAME="descriptor_i050_until4000_30k_r_auto"
BASE_OUT="output/0011/descriptor_i050_until4000"
CONFIG="configs/experiments/0011_descriptor_i050_active_until4000.yaml"
METHOD="descriptor_i050_until4000"

usage() {
  cat <<'EOF'
Usage:
  scripts/run_0011_descriptor_i050_until4000_tmux.sh start
  scripts/run_0011_descriptor_i050_until4000_tmux.sh worker-g0
  scripts/run_0011_descriptor_i050_until4000_tmux.sh worker-g1
  scripts/run_0011_descriptor_i050_until4000_tmux.sh merge
...
existing_pids_for() {
  ...
  ps -eo pid=,cmd= \
    | awk -v group="$group" '
      ...
      $0 ~ "output/0011/descriptor_i050_until4000/" group &&
      $0 !~ "awk" { print $1 }
    ' \
    | tr '\n' ' '
}
...
wait_for_pids() {
  ...
  echo "[0011-u4000] waiting for existing foreground PIDs: $pids"
  for pid in $pids; do
...
archive_incomplete() {
  ...
  if [[ -d "$run_dir" && ! -s "$result_file" ]]; then
    ...
    archive_root="output/0011/debug_artifacts/interrupted_until4000_${stamp}/$(basename "$out_root")/$scene"
    ...
    echo "[0011-u4000] archived incomplete $scene to $archive_root"
  fi
}
...
run_worker() {
  ...
  if [[ "${#missing[@]}" -eq 0 ]]; then
    echo "[0011-u4000] no missing scenes for $out_root"
    return
  fi

  echo "[0011-u4000] GPU $gpu running scenes: ${missing[*]}"
...
base_out = Path("output/0011/descriptor_i050_until4000")
run_name = "descriptor_i050_until4000_30k_r_auto"
method = "descriptor_i050_until4000"
baseline_path = Path("output/0002/phase0_5090_fastgs_big_baseline_fix1/mipnerf360_combined/summary.csv")
until8000_path = Path("output/0010/descriptor_i050_until8000_full9_combined/summary.csv")
...
until8000 = {row["scene"]: row for row in read_rows(until8000_path)}
...
u8000_comp = build_comparison(until8000, "descriptor_i050_until8000")
write_csv(combined / "comparison_vs_phase0.csv", phase0_comp, comparison_fields)
write_csv(combined / "comparison_vs_until8000.csv", u8000_comp, comparison_fields)
...
summary = {
    ...
    "avg_delta_psnr_until8000": avg([row["delta_psnr"] for row in u8000_comp]),
    "avg_delta_ssim_until8000": avg([row["delta_ssim"] for row in u8000_comp]),
    "avg_delta_lpips_until8000": avg([row["delta_lpips"] for row in u8000_comp]),
    "avg_delta_gs_num_until8000": avg([row["delta_gs_num"] for row in u8000_comp]),
    "avg_qcgi_until8000": avg([row["qcgi"] for row in u8000_comp]),
}
...
start_sessions() {
  mkdir -p output/0011/debug_logs
  ...
  if ! tmux has-session -t "$SESSION_G0" 2>/dev/null; then
    tmux new-session -d -s "$SESSION_G0" \
      "cd '$ROOT_DIR' && export WAIT_PIDS='$wait_g0' && bash scripts/run_0011_descriptor_i050_until4000_tmux.sh worker-g0 2>&1 | tee output/0011/debug_logs/${SESSION_G0}.log"
  fi
  if ! tmux has-session -t "$SESSION_G1" 2>/dev/null; then
    tmux new-session -d -s "$SESSION_G1" \
      "cd '$ROOT_DIR' && export WAIT_PIDS='$wait_g1' && bash scripts/run_0011_descriptor_i050_until4000_tmux.sh worker-g1 2>&1 | tee output/0011/debug_logs/${SESSION_G1}.log"
  fi
  if ! tmux has-session -t "$SESSION_MERGE" 2>/dev/null; then
    tmux new-session -d -s "$SESSION_MERGE" \
      "cd '$ROOT_DIR' && while tmux has-session -t '$SESSION_G0' 2>/dev/null || tmux has-session -t '$SESSION_G1' 2>/dev/null; do sleep 60; done; bash scripts/run_0011_descriptor_i050_until4000_tmux.sh merge 2>&1 | tee output/0011/debug_logs/${SESSION_MERGE}.log"
  fi

  tmux ls | grep '0011_i050_u4000' || true
}
