...
SESSION_G0="0021_taming_imp_g0"
SESSION_G1="0021_taming_imp_g1"
SESSION_MERGE="0021_taming_imp_merge"
BASE_OUT="output/0021/taming_importance_fastgs_big_full9"
SCENES_G0="${SCENES_G0:-garden bicycle flowers room}"
SCENES_G1="${SCENES_G1:-kitchen bonsai stump treehill counter}"

CONFIG="configs/experiments/0021_taming_importance_fastgs_big_full9.yaml"
METHOD="taming_importance_fastgs_prune_densify100"
RUN_NAME="taming_importance_fastgs_prune_30k_r_auto"

usage() {
  cat <<'EOF'
Usage:
  scripts/run_0021_taming_importance_fastgs_big_tmux.sh start
  scripts/run_0021_taming_importance_fastgs_big_tmux.sh worker-g0
  scripts/run_0021_taming_importance_fastgs_big_tmux.sh worker-g1
  scripts/run_0021_taming_importance_fastgs_big_tmux.sh merge
...
run_worker() {
  ...
  echo "[0021] GPU $gpu scenes: ${scenes[*]}"
  CUDA_VISIBLE_DEVICES="$gpu" uv run --active python scripts/run_0021_taming_importance_fastgs_big_eval.py \
...
base = Path("output/0021/taming_importance_fastgs_big_full9")
...
start_sessions() {
  mkdir -p output/0021/debug_logs
  if ! tmux has-session -t "$SESSION_G0" 2>/dev/null; then
    tmux new-session -d -s "$SESSION_G0" \
      "cd '$ROOT_DIR' && export SCENES_G0='$SCENES_G0' && bash scripts/run_0021_taming_importance_fastgs_big_tmux.sh worker-g0 2>&1 | tee output/0021/debug_logs/${SESSION_G0}.log"
  fi
  if ! tmux has-session -t "$SESSION_G1" 2>/dev/null; then
    tmux new-session -d -s "$SESSION_G1" \
      "cd '$ROOT_DIR' && export SCENES_G1='$SCENES_G1' && bash scripts/run_0021_taming_importance_fastgs_big_tmux.sh worker-g1 2>&1 | tee output/0021/debug_logs/${SESSION_G1}.log"
  fi
  if ! tmux has-session -t "$SESSION_MERGE" 2>/dev/null; then
    tmux new-session -d -s "$SESSION_MERGE" \
      "cd '$ROOT_DIR' && while tmux has-session -t '$SESSION_G0' 2>/dev/null || tmux has-session -t '$SESSION_G1' 2>/dev/null; do sleep 60; done; bash scripts/run_0021_taming_importance_fastgs_big_tmux.sh merge 2>&1 | tee output/0021/debug_logs/${SESSION_MERGE}.log"
  fi
  tmux ls | grep '0021_taming_imp' || true
}
