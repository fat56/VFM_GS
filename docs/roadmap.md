...
- `0001_vfm_topology_scorer`: 第一版实验进入收束阶段。`cached_edge_l1 + staged target ~= 1.42x baseline count` 已完成 MipNeRF360 全 9 场景评估，平均 PSNR/SSIM/LPIPS 均超过 baseline，是 0001 v1 的正向控制组；但平均 Gaussian 数量增加约 24.5%，训练时间增加约 11.1%，且 `treehill` 是明确负例。Tandt/DB 新增全场景评估显示跨数据集差异：DB 两场景平均正向，Tandt 两场景平均负向且 Gaussian 数量下降约 37.3%。已增加默认关闭的 `prune_min_gaussian_count` 并完成 Tandt 容量保护诊断：平均结果从 cached edge v1 的 PSNR 25.5799、SSIM 0.9316、LPIPS 0.0608 恢复到 25.7806、0.9337、0.0585，但仍低于 baseline 的 25.9551、0.9377、0.0541。DINO token-edge `top-k25 + i0.50` 已完成 MipNeRF360 全 9 场景评估，均值 28.8577 / 0.8666 / 0.1385、263,572 个 Gaussians，相对 baseline 为 +0.2051 PSNR、+0.0115 SSIM、LPIPS -0.0234，是当前全场景质量候选。`weighted + i0.50` 已完成全 9 场景复验，均值 28.8505 / 0.8660 / 0.1397、254,736 个 Gaussians，相比普通 i0.50 少 8,836 点、训练少 2.87s，质量只小幅回落，因此是预算效率候选。`weighted + i0.75` 已补齐 MipNeRF360 全 9 场景，固定均值 28.8396 / 0.8663 / 0.1395、257,715 个 Gaussians，PSNR 低于 i0.50，不作为默认档；但按场景选择 i0.50/i0.75 的 `quality_pick` 均值达到 28.8636 / 0.8665 / 0.1393、257,223 个 Gaussians，相比固定 i0.50 提升 +0.0130 PSNR、+0.0005 SSIM、LPIPS -0.0004，只多 2,487 点。已新增 `0001_vfm_topology_dinov2_token_edge_weighted_i050.yaml` 和 `0001_vfm_topology_dinov2_token_edge_weighted_i075.yaml`，分别作为预算效率档和场景选择质量档的可复用入口；`scripts/summarize_0001_weighted_candidates.py` 也已把 weighted 选择规则做成可重复生成的 summary/comparison/recommendation/recommendation_averages 表。support-ratio、高置信保护、软预算衰减和 adaptive weighted 都已验证但没有超过 weighted 场景选择结论；`adaptive_weighted + quadratic 430k` 在 treehill 第二场景未通过。下一步应把容量保护升级为自动场景自适应机制，并把 weighted 选择规则接入后续批量评估流程。
...
- 下一版优先设计预算感知 scorer 和自动容量保护，而不是继续堆叠 descriptor mask ratio：候选方向包括当自然点数显著低于 baseline 预测值时回退 pruning 强度、在 staged pruning 后立即短恢复而非训练结束统一恢复。按 Gaussian 支持度归一化 VFM score 和高置信 VFM 区域保护都已完成 bicycle 30k 验证，均不优于普通 i0.50；`weighted` importance 已完成 MipNeRF360 全 9 场景 i0.50 与 i0.75 复验。固定 i0.50 是预算效率默认候选，固定 i0.75 均值不优；按场景选择的 `quality_pick` 为 28.8636 / 0.8665 / 0.1393、257,223 个 Gaussians，相比 i0.50 三项质量均正向且只多 2,487 个点。`prune_min_gaussian_target_ratio` 已把手工容量下限推进为默认关闭的自动派生机制；下一步考虑让 ratio 来自 baseline 预跑或在线增长曲线。后续不再优先做同类手工单点，改为固化选择规则：预算优先选 weighted i0.50，质量优先且 QCGI 为正时允许小幅或中等 GS 增长；单场景负例只触发该场景回退，不直接否定整个方法；当 GS 增长超过 0.1M 且 QCGI 不足时，优先回退 baseline/cached-edge 或增强 pruning。
