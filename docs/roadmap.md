...
- `0001 cross-dataset selector`: 已新增 `scripts/summarize_0001_cross_dataset_selector.py`，汇总 MipNeRF360/Tandt/DB 共 13 个场景的 baseline、cached-edge v1 和 DINO weighted i0.50。整体平均上，DINO weighted i0.50 相对 baseline 为 +0.1430 PSNR、+0.0076 SSIM、LPIPS -0.0152；相对 cached-edge v1 为 +0.0848 PSNR、+0.0061 SSIM、LPIPS -0.0112。逐场景 PSNR 最优分布为 8 个 DINO weighted、2 个 cached-edge、3 个 baseline；当前保守 `validated_policy` 与 PSNR oracle 一致，均值为 28.6786 / 0.8868 / 0.1182，保守预算选择均值为 28.4887 / 0.8800 / 0.1302、135,162 个 Gaussians。已新增 QCGI（质量-容量收益指数）作为离线实验准则：`quality_gain = ΔPSNR + 20*ΔSSIM - 5*ΔLPIPS`，并对 0.1M 以上 GS 增长加重惩罚；当前 QCGI 选择也与 PSNR oracle 一致。下一步主线应从固定后端转向场景级选择与自动回退，并评估能否把 QCGI 分档接入自适应 density/prune 强度。
...
- 下一版优先设计预算感知 scorer 和自动容量保护，而不是继续堆叠 descriptor mask ratio：候选方向包括当自然点数显著低于 baseline 预测值时回退 pruning 强度、在 staged pruning 后立即短恢复而非训练结束统一恢复。按 Gaussian 支持度归一化 VFM score 和高置信 VFM 区域保护都已完成 bicycle 30k 验证，均不优于普通 i0.50；`weighted` importance 已完成 MipNeRF360 全 9 场景 i0.50/i0.75/i0.90 复验。固定 i0.50 是预算效率默认候选，固定 i0.75/i0.90 均值不优；严格 `quality_pick` 为 28.8641 / 0.8665 / 0.1392、257,326 个 Gaussians，`QCGI pick` 为 28.8641 / 0.8667 / 0.1388、255,822 个 Gaussians，相比 weighted i0.50 三项质量均正向且只多 1,086 个点。`prune_min_gaussian_target_ratio` 已把手工容量下限推进为默认关闭的自动派生机制；下一步考虑让 ratio 来自 baseline 预跑或在线增长曲线。后续不再优先做同类手工单点，改为固化选择规则：预算优先选 weighted i0.50，质量优先且 QCGI 为正时允许小幅或中等 GS 增长；单场景负例只触发该场景回退，不直接否定整个方法；当 GS 增长超过 0.1M 且 QCGI 不足时，优先回退 baseline/cached-edge 或增强 pruning。
