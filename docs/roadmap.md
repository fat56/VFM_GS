...
- `0001 semantic prior quality`: 研究目标已从“效果不好回退 FastGS”修订为“证明 VFM 先验能带来质量提升”。下一步优先验证 DINO descriptor residual 是否能单独指导复制：新增配置 `0001_vfm_topology_dinov2_descriptor_densify_only.yaml`，使用 `dinov2_descriptor_cosine`、top-k 15%、token smoothing 3、`vfm_weight=0.0`、`vfm_importance_mode=max`，只影响 densification，不改变 pruning score。第一批 `-r 8` MipNeRF360 小范围场景为 bicycle、garden、stump、bonsai；若相对 `fastgs_densify100` 或 token-edge weighted 出现清晰质量收益，再扩展高分辨率三数据集。Depth Anything 暂列为下一阶段几何先验，用于 scale/shift 对齐后的 depth residual 和 depth edge protect。
...
- `0001 大分辨率基线复核`: 已完成 MipNeRF360、DB、Tandt 三个公开数据集的 `fastgs_big/densify100` 与 `VFM + FastGS big 场景超参` 同 recipe 对照。MipNeRF360 VFM+big 为 27.9379 / 0.8199 / 0.2148、1,193,697 个 Gaussians，相对基线 +0.0086 PSNR、+0.0002 SSIM、LPIPS -0.0009，平均多 32,455 个点；DB 为 30.2236 / 0.9108 / 0.2362，相对基线 +0.0163 PSNR、-0.0004 SSIM、LPIPS -0.0040，平均多 90,192 个点；Tandt 为 24.3691 / 0.8569 / 0.1739，相对基线 +0.0134 PSNR、-0.0004 SSIM、LPIPS -0.0006，平均少 23,262 个点。结论：迁移和评测链路健康，recipe 对齐后 VFM 大分辨率跨数据集具备平均弱正向证据，但提升很薄，bicycle/garden、DB drjohnson、Tandt truck 等负例或高增点场景需要后续选择/回退机制。
...
- Depth Anything 几何先验作为 v2 质量主线排队：先离线缓存 GT monocular depth，再对 SH0 渲染图预测相对深度，使用 scale/shift 或 rank 对齐计算 depth residual，并用 depth edge 区域做 densification boost 与 prune-protect。该路线需要先确认依赖、显存、推理速度和相对深度对齐稳定性，不与 DINO descriptor densify-only 第一批实验混在一起。
...
- 大分辨率下一步不继续扩大 DINO 模型；优先做容量收益门槛和不泄漏 test 的回退规则：当 `ΔGS < 0.1M` 且质量正向时保留 VFM，当 `ΔGS >= 0.1M` 但 QCGI 不足或 PSNR/SSIM/LPIPS 混合负向时回退 FastGS 或降低 VFM 介入强度。短期开发集可优先使用 bicycle/garden、DB drjohnson、Tandt truck，因为它们分别代表负例、接近 0.1M 以上增点和混合指标。
