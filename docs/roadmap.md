...
- `0001 adaptive weighted`: 已在 bicycle 上完成 `adaptive_weighted + quadratic 430k` 30k 对照，结果为 PSNR 26.9858、SSIM 0.8302、LPIPS 0.1853、424,011 个 Gaussians。相比 `weighted i0.50` 三项质量微升但多 8,853 个点；相比普通 i0.50 少 16,060 个点但质量仍小幅低一点。下一步优先跑 `stump/room/kitchen/treehill` 复验，确认它能否从 bicycle 单场景候选升级为全场景预算效率候选。
- `0001_vfm_topology_scorer`: 第一版实验进入收束阶段。`cached_edge_l1 + staged target ~= 1.42x baseline count` 已完成 MipNeRF360 全 9 场景评估，平均 PSNR/SSIM/LPIPS 均超过 baseline，是 0001 v1 的正向控制组；但平均 Gaussian 数量增加约 24.5%，训练时间增加约 11.1%，且 `treehill` 是明确负例。Tandt/DB 新增全场景评估显示跨数据集差异：DB 两场景平均正向，Tandt 两场景平均负向且 Gaussian 数量下降约 37.3%。已增加默认关闭的 `prune_min_gaussian_count` 并完成 Tandt 容量保护诊断：平均结果从 cached edge v1 的 PSNR 25.5799、SSIM 0.9316、LPIPS 0.0608 恢复到 25.7806、0.9337、0.0585，但仍低于 baseline 的 25.9551、0.9377、0.0541。下一步应把容量保护升级为自动场景自适应机制，并继续处理 edge/cadence/pruning trajectory 的耦合。DINOv2 descriptor 路径已从 cache、在线 descriptor、top-k/smoothing、soft top-k、percentile、`rgb_only`、staged 预算对齐推进到 dense recovery；当前结论是 descriptor unpruned 有真实质量收益，但接近 410k budget、降低 top-k ratio、只参与 support/pruning、percentile mask、多层 soft 计数、严格 240k dense recovery 或 staged 后 dense recovery 后，仍未稳定超过 `fastgs_densify100` cadence control。DINO token-edge top-k 25% 完成 bicycle 30k 对照，达到 PSNR 27.0636、SSIM 0.8354、LPIPS 0.1748，刷新当前 bicycle 质量上界；staged 490,832 预算版回落到 27.0001 / 0.8286 / 0.1887，low-score final 490,832 只裁 6,723 个点仍回落到 26.8466 / 0.8244 / 0.1858，high-score final 490,832 只裁 5,432 个点但崩到 24.0554 / 0.7914 / 0.2040，`rgb_only` 虽贴近 cadence control 点数但只有 26.9340 / 0.8236 / 0.1981。新的 partial importance 曲线持续正向：`importance_weight=0.25` 达到 26.9515 / 0.8262 / 0.1920、420,361 个 Gaussians；`importance_weight=0.50` 达到 26.9966 / 0.8303 / 0.1842、440,071 个 Gaussians，是当前约 440k 点数带的效率点；`importance_weight=0.75` 达到 27.0284 / 0.8332 / 0.1788、472,164 个 Gaussians，略优于 top-k 15%，是高质量预算点。`garden` 第二场景复验中，top-k 25% + `importance_weight=0.50` 达到 28.9644 / 0.8986 / 0.0954、262,385 个 Gaussians，超过 garden baseline 和 cached-edge v1，说明该预算效率点不只在 bicycle 生效。`counter` 第三场景复验达到 29.7174 / 0.9338 / 0.0751、119,695 个 Gaussians，相比 counter baseline 三项指标均改善且点数只增加约 5.9%，是当前最接近预算克制的跨场景正例。`treehill` 压力测试达到 24.5173 / 0.7284 / 0.2822、432,520 个 Gaussians，相比 baseline PSNR 低 -0.0344，但 SSIM/LPIPS 大幅正向；`treehill weighted` 达到 24.5101 / 0.7281 / 0.2837、417,534 个 Gaussians，少 14,986 个点且基本保住普通 i0.50 的压力场景收益，但仍没有解决相对 baseline 的 PSNR 负向。`bonsai` 复验达到 32.4920 / 0.9640 / 0.0500、136,305 个 Gaussians，相比 baseline 和 cached-edge v1 三项指标均正向。`flowers` 复验达到 23.0134 / 0.6960 / 0.2747、350,421 个 Gaussians，相比 baseline 和 cached-edge v1 也三项正向。`kitchen` 复验达到 33.3358 / 0.9693 / 0.0344、161,347 个 Gaussians，相比 baseline 和 cached-edge v1 三项指标均正向，且点数低于 baseline。`room` 复验达到 33.0721 / 0.9622 / 0.0574、103,820 个 Gaussians，相比 baseline 和 cached-edge v1 三项指标均正向，且训练比 cached-edge v1 更快。`stump` 复验达到 27.6106 / 0.8168 / 0.1935、365,584 个 Gaussians，相比 baseline 和 cached-edge v1 三项指标均正向，是全场景中 PSNR 增益最大的 DINO i0.50 正例。九场景 candidate 均值达到 28.8577 / 0.8666 / 0.1385、263,572 个 Gaussians；相对 baseline 为 +0.2051 PSNR、+0.0115 SSIM、LPIPS -0.0234，相对 cached-edge v1 为 +0.1365 PSNR、+0.0087 SSIM、LPIPS -0.0166，但平均多 90,231 个 Gaussians。`vfm_importance_normalizer=support_ratio` 30k bicycle 对照为 26.9694 / 0.8286 / 0.1878、432,948 个 Gaussians；它比 i0.50 少 7,123 个点但质量回落，因此支持度归一化不是当前最佳预算效率方向。高置信 VFM 区域保护 30k bicycle 对照为 26.9910 / 0.8302 / 0.1839、441,352 个 Gaussians；它与普通 i0.50 基本持平但没有超过，因此不作为下一步主方向。`weighted + importance_weight=0.50` bicycle 对照达到 26.9756 / 0.8288 / 0.1867、415,158 个 Gaussians，点数几乎贴近 cadence control，同时保住相对 cadence 的 +0.0469 PSNR、+0.0047 SSIM 和 -0.0097 LPIPS；`stump weighted` 达到 27.6147 / 0.8170 / 0.1934、354,046 个 Gaussians，相比普通 stump i0.50 少 11,538 个点且质量基本持平；`counter weighted` 达到 29.6650 / 0.9333 / 0.0752、119,273 个 Gaussians，相比 baseline 和 cached-edge v1 仍正向，但相对普通 counter i0.50 省点很少且质量回落；`garden weighted` 达到 28.9546 / 0.8977 / 0.0974、253,355 个 Gaussians，相比普通 garden i0.50 少 9,030 个点且仍超过 baseline/cached-edge；`flowers weighted` 达到 22.9636 / 0.6933 / 0.2791、339,267 个 Gaussians，相比普通 flowers i0.50 少 11,154 个点但质量回落更明显，仍优于 baseline 且保住相对 cached-edge 的 SSIM/LPIPS 改善；`bonsai weighted` 达到 32.5395 / 0.9642 / 0.0493、134,806 个 Gaussians，相比普通 bonsai i0.50 少 1,499 个点且质量微升。weighted 是新的近预算效率点，但适用边界与场景点数增长幅度、纹理复杂度和 DINO topology 对结构选择的实际帮助相关。

## 排队

- 下一版优先设计预算感知 scorer 和自动容量保护，而不是继续堆叠 descriptor mask ratio：候选方向包括当自然点数显著低于 baseline 预测值时回退 pruning 强度、在 staged pruning 后立即短恢复而非训练结束统一恢复。按 Gaussian 支持度归一化 VFM score 和高置信 VFM 区域保护都已完成 bicycle 30k 验证，均不优于普通 i0.50；`weighted` importance 已完成 MipNeRF360 全 9 场景复验，平均 PSNR 28.8505、SSIM 0.8660、LPIPS 0.1397、254,736 个 Gaussians、训练 137.60s。相比普通 i0.50，它平均少 8,836 个点、训练少 2.87s，质量只小幅回落 -0.0072 PSNR、-0.0006 SSIM、LPIPS 差 +0.0012。因此 weighted 选择规则可以固化为下一版计划：普通 i0.50 作为质量上界，weighted 作为预算效率选项。
...
- 已增加 `adaptive_weighted` importance mode，用同一个预算进度在 `max` partial importance 和 `weighted` importance 之间平滑过渡。bicycle 620-step 快速验证完成 train/render/metrics；30k linear 430k 对照为 26.9724 / 0.8292 / 0.1859、421,472 个 Gaussians，属于混合结果。quadratic 430k 对照为 26.9858 / 0.8302 / 0.1853、424,011 个 Gaussians，是新的 bicycle 单场景预算效率候选。下一步只做跨场景复验，不继续在 bicycle 上密集扫相近曲线。
