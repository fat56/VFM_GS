...
- 若继续做 0001 追加验证，只保留 full-scene 汇总或明确改变预算行为的实验，不再追加同类 top-k / percentile / soft-top-k 单点，也不再追加单纯 final hard-prune 预算点。DINO i0.50 已完成 `bicycle`、`garden`、`counter`、`treehill`、`bonsai`、`flowers`、`kitchen` 和 `room`，下一步补齐 MipNeRF360 的 `stump` 后汇总成 v1 candidate 表，并设计自动容量保护。
