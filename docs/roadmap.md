...
- 大分辨率下一步先补“VFM + FastGS big 场景级超参”的 MipNeRF360 对照。当前 VFM 大分辨率虽为 densify100，但没有套 `train_big.sh` 的每场景 `dense/grad_abs_thresh/highfeature_lr` 设置；在这个公平对照完成前，不再把 27.2965 作为最终大分辨率方法结论。如果公平对照仍低于 FastGS big，则优先降低 VFM pruning/density 介入强度或接入容量回退，而不是继续扩大 DINO 模型。
