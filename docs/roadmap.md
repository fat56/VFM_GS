...
- 下一版优先设计预算感知 scorer 和自动容量保护，而不是继续堆叠 descriptor mask ratio：候选方向包括当自然点数显著低于 baseline 预测值时回退 pruning 强度、在 staged pruning 后立即短恢复而非训练结束统一恢复。按 Gaussian 支持度归一化 VFM score 和高置信 VFM 区域保护都已完成 bicycle 30k 验证，均不优于普通 i0.50；`weighted` importance 已完成 MipNeRF360 全 9 场景 i0.50 复验，平均 PSNR 28.8505、SSIM 0.8660、LPIPS 0.1397、254,736 个 Gaussians、训练 137.60s。相比普通 i0.50，它平均少 8,836 个点、训练少 2.87s，质量只小幅回落 -0.0072 PSNR、-0.0006 SSIM、LPIPS 差 +0.0012。`weighted i0.75` 已在 bicycle/stump/room 复验为 PSNR 正向，但 bonsai 未超过 weighted i0.50。`prune_min_gaussian_target_ratio` 已把手工容量下限推进为默认关闭的自动派生机制；下一步考虑让 ratio 来自 baseline 预跑或在线增长曲线。后续不再优先做同类手工单点，改为固化选择规则：预算优先选 weighted i0.50，质量优先且 QCGI 为正时允许小幅或中等 GS 增长；当 GS 增长超过 0.1M 且 QCGI 不足时，优先回退 baseline/cached-edge 或增强 pruning。
