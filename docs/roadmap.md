...
- `0001 Tandt DINO weighted`: 已完成 Tandt `train/truck` 的 `dinov2_token_edge_l1 + top-k25 + weighted i0.50/i0.75/i0.90` 复验。i0.50 平均为 25.7519 / 0.9346 / 0.0575、38,394 个 Gaussians，仍是 Tandt 三档里最好的 DINO weighted 候选；i0.75 为 25.6201 / 0.9328 / 0.0585、37,986 个 Gaussians，i0.90 为 25.5329 / 0.9323 / 0.0611、37,523 个 Gaussians。高权重档位没有修复 Tandt 相对 baseline 的差距，反而降低 PSNR 并显著拉长训练时间。DINO weighted i0.50 + 自动容量下限也已完成，最终点数被拉回 baseline 均值 50,370，但平均只有 25.6078 / 0.9324 / 0.0610，低于原始 i0.50 和 baseline；训练日志显示早期 staged target 已大幅改写结构轨迹。因此 Tandt 当前应回退 baseline，DINO weighted 只记录为相对 cached-edge v1 的恢复型 VFM 分支，不作为默认方案。
...
- 已新增 `prune_min_gaussian_target_ratio`，默认关闭。它会在训练开始时从 `target_gaussian_count` 自动派生有效 `prune_min_gaussian_count`，并写入 `cfg_args`；显式 `prune_min_gaussian_count` 仍优先。配套配置 `0001_vfm_topology_cached_edge_auto_prunemin.yaml` 使用 ratio `0.7042253521126761`，当 target 仍取 `baseline * 1.42` 时可自动还原 baseline 容量下限。Tandt `train/truck` 自动下限复验已完成，cached-edge 平均 PSNR/SSIM/LPIPS 为 25.7473 / 0.9330 / 0.0594，DINO weighted i0.50 + 自动容量下限平均为 25.6078 / 0.9324 / 0.0610，二者最终点数均为 50,370。结论是机制可用，但容量下限只适合作为防线；它不能修复早期 staged target/pruning trajectory 对结构分布造成的损伤。

## 排队

- 下一版优先把离线 selector 固化为可复现实验入口，但不能使用 test oracle。短期更可行的方向是：一方面用独立 validation split 或预先固定的数据集级策略评估 selector，另一方面用训练过程信号做保守回退。Tandt 的 DINO weighted + 自动容量下限已经证明容量不足不是主因，因此后续不再继续最终补容量支线；若继续修复 Tandt，应优先测试推迟/取消早期 staged target、降低早期 VFM 介入，或直接回退 baseline。随后再把 QCGI 分档接入自适应 density/prune 强度：当候选 `ΔGS < 0.1M` 且 QCGI 为正时保留增长，当 `ΔGS >= 0.1M` 且 QCGI 不足时回退或增强 pruning。
