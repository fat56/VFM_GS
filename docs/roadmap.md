...
- `0001 大分辨率基线复核`: 已补跑 MipNeRF360 `fastgs_big/densify100` 同口径基线，训练/渲染/测试均为 `-i images -r -1`，沿用 FastGS 1.6K 自动缩放，并复用 `scripts/train_big.sh` 的场景级超参。结果为 PSNR 27.9293、SSIM 0.8198、LPIPS 0.2157、平均 1,161,242 个 Gaussians，和用户提供的论文参考 27.90 基本一致。未套场景超参的大分辨率 ViT-L token-edge i0.50 为 27.2965 / 0.8017 / 0.2506、581,266 个 Gaussians，低 -0.6328 PSNR；补上 FastGS big 场景超参后，VFM 达到 27.9379 / 0.8199 / 0.2148、1,193,697 个 Gaussians，相对基线为 +0.0086 PSNR、+0.0002 SSIM、LPIPS -0.0009，平均多 32,455 个点。结论：迁移和评测链路健康，recipe 对齐后 VFM 大分辨率在 MipNeRF360 上小幅正向，但 bicycle/garden 负例和若干场景超过 0.1M 的 GS 增长需要后续选择/回退机制。
...
- 大分辨率下一步补 DB/Tandt 的 `fastgs_big/densify100` 与 `VFM + scene overrides` 同 recipe 对照，并把 MipNeRF360 的 bicycle/garden 作为回退规则开发集。当前不继续扩大 DINO 模型；优先做容量收益门槛：当 `ΔGS < 0.1M` 且质量正向时保留 VFM，当 `ΔGS >= 0.1M` 但 QCGI 不足或 PSNR/SSIM/LPIPS 混合负向时回退 FastGS 或降低 VFM 介入强度。
