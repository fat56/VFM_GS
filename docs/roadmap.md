...
- `0001 Tandt DINO weighted`: 已完成 Tandt `train/truck` 的 `dinov2_token_edge_l1 + top-k25 + weighted i0.50/i0.75/i0.90` 复验。i0.50 平均为 25.7519 / 0.9346 / 0.0575、38,394 个 Gaussians，仍是 Tandt 三档里最好的 DINO weighted 候选；i0.75 为 25.6201 / 0.9328 / 0.0585、37,986 个 Gaussians，i0.90 为 25.5329 / 0.9323 / 0.0611、37,523 个 Gaussians。高权重档位没有修复 Tandt 相对 baseline 的差距，反而降低 PSNR 并显著拉长训练时间。DINO weighted i0.50 + 自动容量下限也已完成，最终点数被拉回 baseline 均值 50,370，但平均只有 25.6078 / 0.9324 / 0.0610，低于原始 i0.50 和 baseline；取消 staged target、仅保留容量下限后，平均为 25.6430 / 0.9341 / 0.0566、50,370 个 Gaussians，LPIPS/SSIM 相对 staged 版本恢复，但 PSNR 仍低于原始 i0.50 和 baseline。因此 Tandt 当前应回退 baseline，DINO weighted 只记录为相对 cached-edge v1 的恢复型 VFM 分支，不作为默认方案。
...
- 下一版优先把离线 selector 固化为可复现实验入口，但不能使用 test oracle。短期更可行的方向是：一方面用独立 validation split 或预先固定的数据集级策略评估 selector，另一方面用训练过程信号做保守回退。Tandt 的 staged 自动容量下限和 prunemin-only 诊断已经证明容量不足不是主因，早期 staged target 会伤感知指标但取消它仍不能转正；后续不再继续最终补容量支线。若继续修复 Tandt，应优先降低早期 VFM 介入，或直接回退 baseline。随后再把 QCGI 分档接入自适应 density/prune 强度：当候选 `ΔGS < 0.1M` 且 QCGI 为正时保留增长，当 `ΔGS >= 0.1M` 且 QCGI 不足时回退或增强 pruning。
