...
- 下一版优先把离线 selector 固化为可复现实验入口，而不是继续追加固定权重单点：预算优先时用 weighted i0.50，质量优先时在 i0.50/i0.75/i0.90 中按 QCGI 或严格质量门槛选；Tandt 这类负例直接回退 baseline，DB/MipNeRF360 的正例允许小幅或中等 GS 增长。随后再把 QCGI 分档接入自适应 density/prune 强度：当候选 `ΔGS < 0.1M` 且 QCGI 为正时保留增长，当 `ΔGS >= 0.1M` 且 QCGI 不足时回退或增强 pruning。`prune_min_gaussian_target_ratio` 已把手工容量下限推进为默认关闭的自动派生机制；下一步考虑让 ratio 来自 baseline 预跑或在线增长曲线。
