...
- `0001 Tandt DINO weighted`: 已完成 Tandt `train/truck` 的 `dinov2_token_edge_l1 + top-k25 + weighted i0.50/i0.75/i0.90` 复验。i0.50 平均为 25.7519 / 0.9346 / 0.0575、38,394 个 Gaussians，仍是 Tandt 三档里最好的 DINO weighted 候选；i0.75 为 25.6201 / 0.9328 / 0.0585、37,986 个 Gaussians，i0.90 为 25.5329 / 0.9323 / 0.0611、37,523 个 Gaussians。高权重档位没有修复 Tandt 相对 baseline 的差距，反而降低 PSNR 并显著拉长训练时间。结论是 Tandt 当前应回退 baseline；DINO weighted 只记录为相对 cached-edge v1 的恢复型 VFM 分支，不作为默认方案。
...
- 下一版优先把离线 selector 固化为可复现实验入口，但不能使用 test oracle。短期更可行的方向是：一方面用独立 validation split 或预先固定的数据集级策略评估 selector，另一方面继续修复 Tandt 负例。Tandt 目前 DINO weighted i0.50 比 cached-edge 好但点数仍低于 baseline，下一轮优先测试 DINO weighted + 自动容量下限，判断容量不足是否是它低于 baseline 的主要原因。随后再把 QCGI 分档接入自适应 density/prune 强度：当候选 `ΔGS < 0.1M` 且 QCGI 为正时保留增长，当 `ΔGS >= 0.1M` 且 QCGI 不足时回退或增强 pruning。
