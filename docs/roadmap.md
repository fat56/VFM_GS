...
- 若继续做 0001 追加验证，只保留跨场景复验或明确改变预算行为的实验，不再追加同类 top-k / percentile / soft-top-k 单点，也不再追加单纯 final hard-prune 预算点。跨场景复验优先选 `counter` 或 `treehill`：前者检查室内/台面场景是否延续收益，后者专门验证 cached-edge v1 负例上 DINO token-edge 是否更稳。
