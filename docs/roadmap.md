...
- `0001 cross-dataset selector`: `scripts/summarize_0001_cross_dataset_selector.py` 已扩展为读取 DINO weighted i0.50/i0.75/i0.90。13 场景固定三档均值分别为 i0.50 28.6061 / 0.8873 / 0.1154、i0.75 28.6066 / 0.8872 / 0.1153、i0.90 28.5919 / 0.8872 / 0.1154。固定档位差异不大，但场景级 `validated_policy` 与 PSNR oracle 一致，均值提升到 28.6981 / 0.8872 / 0.1179、178,903 个 Gaussians，相对 baseline 为 +0.2350 PSNR、+0.0075 SSIM、LPIPS 改善 -0.0127。QCGI pick 均值为 28.6930 / 0.8881 / 0.1147、188,542 个 Gaussians，更偏 SSIM/LPIPS。逐场景 PSNR 最优分布更新为 9 个 DINO weighted、1 个 cached-edge、3 个 baseline。下一步主线应从固定后端转向“自动场景选择/回退 + QCGI 容量约束”。
...
- `0001 dataset policies`: 新增 `scripts/summarize_0001_dataset_policies.py`，把当前收束出的策略固化成非 oracle 展示线。`dataset_fixed_policy` 使用 MipNeRF360 fixed weighted i0.50、DB fixed DINO weighted i0.90、Tandt baseline，13 场景均值为 28.6754 / 0.8881 / 0.1146、193,798 个 Gaussians，相对 baseline 为 +0.2123 PSNR、+0.0083 SSIM、LPIPS -0.0160。`dataset_quality_policy` 使用 MipNeRF360 weighted QCGI、DB fixed i0.90、Tandt baseline，均值为 28.6848 / 0.8886 / 0.1140，相对 baseline 为 +0.2217 PSNR、+0.0088 SSIM、LPIPS -0.0166。
