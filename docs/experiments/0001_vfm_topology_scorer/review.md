...
1. 补 `max_width=518` 或 `640` 的 DINO ViT-S/14 cache build/validate，并用 `vfm_loss_thresh=0.35` 复测 descriptor 快速验证。
2. 如果高分辨率 cache 仍稳定，再跑 30k `-r 8`，并与 token-edge、edge 正向控制组、no-effect/cadence 控制同表比较。
