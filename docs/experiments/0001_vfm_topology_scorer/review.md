...
跨数据集选择汇总脚本已把 13 个场景的 baseline、cached-edge v1 和 DINO weighted i0.50 放到同一张表。整体平均上，DINO weighted i0.50 相对 baseline 提升 +0.1430 PSNR、+0.0076 SSIM、LPIPS 改善 -0.0152；相对 cached-edge v1 提升 +0.0848 PSNR、+0.0061 SSIM、LPIPS 改善 -0.0112。但逐场景 PSNR 最优分布为：8 个场景选 DINO weighted i0.50，2 个场景选 cached-edge v1，3 个场景选 baseline。当前新增的 QCGI 指数用 `ΔPSNR + 20*ΔSSIM - 5*ΔLPIPS` 作为质量收益，并对 0.1M 以上 GS 增长加重惩罚；它在现有 13 场景上与 PSNR oracle 和 `validated_policy` 一致，均值为 28.6786 / 0.8868 / 0.1182。weighted 三档汇总也已接入 QCGI：严格 `quality_pick` 均值为 28.8641 / 0.8665 / 0.1392、257,326 个 Gaussians，`QCGI pick` 均值为 28.8641 / 0.8667 / 0.1388、255,822 个 Gaussians。保守预算选择则为 28.4887 / 0.8800 / 0.1302、135,162 个 Gaussians。下一版应把 0001 收束为“场景级后端选择/自动回退”问题，并把 QCGI 作为后续自适应 density/prune 强度的候选指导信号。
