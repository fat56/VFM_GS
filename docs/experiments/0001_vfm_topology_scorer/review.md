...
12. DINO descriptor top-k25 质量优先档已完成 MipNeRF360 全 9 场景复验。它保持 `vfm_weight=0.0`，只把 descriptor metric map 从 top-k15 扩到 top-k25，并且只影响 densification、不改变 pruning score。全 9 场景相对 matched FastGS densify100 平均提升 +0.1066 PSNR、+0.0050 SSIM、LPIPS 改善 -0.0093，平均 Gaussian 数量增加 50,131；9/9 场景三项指标全部正向。该结果是当前 descriptor densify-only 的质量优先主证据，但 `bicycle` 和 `stump` 单场景增点分别为 +105,764 和 +144,738，说明下一步仍要做容量收益门槛或自适应 importance，而不能把 top-k25 max 直接作为预算效率结论。
