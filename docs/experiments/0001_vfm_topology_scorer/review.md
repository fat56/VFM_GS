...
- `quality_pick` 按场景在 i0.50/i0.75 之间选择，当前选中 bicycle、counter、garden、room、stump 的 i0.75，其余场景保留 i0.50。全 9 场景均值为 PSNR 28.8636、SSIM 0.8665、LPIPS 0.1393、257,223 个 Gaussians，相比固定 i0.50 提升 +0.0130 PSNR、+0.0005 SSIM、LPIPS 改善 -0.0004，只多 2,487 点。它是当前 weighted 分支最值得保留的质量策略。
...
6. `weighted + importance_weight=0.50` 已完成全 9 场景复验，是预算效率默认候选；`weighted + importance_weight=0.75` 已补齐全 9 场景，固定均值不超过 i0.50，但按场景选择后的 `quality_pick` 三项质量均优于固定 i0.50。下一步把规则收束为：预算优先选 `weighted i0.50`；质量优先时用 QCGI 或同等质量门槛允许小幅 GS 增长，选择 `weighted i0.75`；单个场景负例只作为回退信号，不再直接否定整个方法，最终以数据集平均和场景分布判断。
