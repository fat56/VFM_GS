...
跨数据集选择汇总脚本已把 13 个场景的 baseline、cached-edge v1 和 DINO weighted i0.50 放到同一张表。整体平均上，DINO weighted i0.50 相对 baseline 提升 +0.1430 PSNR、+0.0076 SSIM、LPIPS 改善 -0.0152；相对 cached-edge v1 提升 +0.0848 PSNR、+0.0061 SSIM、LPIPS 改善 -0.0112。但逐场景 PSNR 最优分布为：8 个场景选 DINO weighted i0.50，2 个场景选 cached-edge v1，3 个场景选 baseline。当前新增的 QCGI 指数用 `ΔPSNR + 20*ΔSSIM - 5*ΔLPIPS` 作为质量收益，并对 0.1M 以上 GS 增长加重惩罚；它在现有 13 场景上与 PSNR oracle 和 `validated_policy` 一致，均值为 28.6786 / 0.8868 / 0.1182。保守预算选择则为 28.4887 / 0.8800 / 0.1302、135,162 个 Gaussians。下一版应把 0001 收束为“场景级后端选择/自动回退”问题，并把 QCGI 作为后续自适应 density/prune 强度的候选指导信号。
...
- `weighted + importance_weight=0.75` bicycle 高质量档位探测最终 414,563 个 Gaussians，PSNR 26.9909、SSIM 0.8309、LPIPS 0.1844，训练 141.48s。相比 `weighted i0.50`，点数少 595 个但 PSNR 高 +0.0153、SSIM 高 +0.0021、LPIPS 改善 -0.0023；相比普通 i0.75，少 57,601 个点、训练少 13.70s，但质量小幅回落。它把 weighted 从 i0.50 扩展到高质量档位，值得优先在 `stump/room` 复验。
- `weighted + importance_weight=0.75` stump 复验最终 370,556 个 Gaussians，PSNR 27.6183、SSIM 0.8178、LPIPS 0.1929，训练 138.82s。相比 `stump weighted i0.50` 多 16,510 个点、训练多 2.46s，但 PSNR 高 +0.0036、SSIM 高 +0.0008、LPIPS 改善 -0.0005；相比普通 stump i0.50 也小幅质量正向。它说明高质量档位可复现，但预算效率不如 i0.50 清晰。
- `weighted + importance_weight=0.75` room 复验最终 101,965 个 Gaussians，PSNR 33.1334、SSIM 0.9626、LPIPS 0.0575，训练 120.08s。相比 `room weighted i0.50` 只多 581 个点、训练少 11.90s，PSNR 高 +0.0253、SSIM 基本持平、LPIPS 极小幅回落 +0.0001；相比普通 room i0.50 少 1,855 个点、训练少 13.66s，PSNR 高 +0.0613。它说明高质量档位在室内小场景也成立，但 LPIPS 不一定同步改善。
- `weighted + importance_weight=0.75` bonsai 边界复验最终 138,808 个 Gaussians，PSNR 32.4848、SSIM 0.9642、LPIPS 0.0497，训练 138.14s。相比 `bonsai weighted i0.50` 多 4,002 个点，PSNR 低 -0.0547、LPIPS 差 +0.0004；相比 baseline 与 cached-edge v1 仍明显正向。这说明 bonsai 的推荐档位应保留 `weighted i0.50`，i0.75 需要选择规则约束。
...
2. 把 `prune_min_gaussian_count` 保留为诊断/回退保护，同时使用默认关闭的 `prune_min_gaussian_target_ratio` 从 staged target 自动派生容量下限。当前 ratio `0.7042253521126761` 在 target 为 `baseline * 1.42` 时等价于 baseline 最终点数，Tandt 复验已确认机制可复现容量保护；下一版不要长期依赖人工填 baseline count，应继续转向 baseline 预跑、在线增长曲线或 scene scale 自动估计容量下限。
...
6. `weighted + importance_weight=0.50` 已完成全 9 场景复验。它平均只比普通 i0.50 低 -0.0072 PSNR、-0.0006 SSIM、LPIPS 差 +0.0012，但少 8,836 个 Gaussians、训练少 2.87s。下一步把规则收束为：默认保留普通 i0.50 作为质量上界；若场景目标更偏预算效率或 DINO topology 已显示结构收益，则选择 `weighted i0.50`；若目标偏高质量、PSNR 权重更高，并且可接受 LPIPS 或点数的小幅波动，再选择 `weighted i0.75`；如果普通 i0.50 已接近 baseline 点数且 quality margin 很小，优先不切换。bonsai 已证明 i0.75 并不总是优于 weighted i0.50，因此选择规则不能只按更大 `importance_weight` 单调外推。
