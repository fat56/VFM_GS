...
12. DINO descriptor top-k25 质量优先档已完成 MipNeRF360、DB、Tandt 三个公开数据集复验。它保持 `vfm_weight=0.0`，只把 descriptor residual 用于 densification，不改变 pruning score。MipNeRF360 全 9 场景平均提升 +0.1066 PSNR、+0.0050 SSIM、LPIPS 改善 -0.0093，9/9 场景三项指标全部正向；DB 平均弱正向但 `playroom` 单场景负向；Tandt 平均提升 +0.1004 PSNR、+0.0017 SSIM、LPIPS 改善 -0.0016，且两场景全部正向。该结果是当前 descriptor densify-only 的跨数据集主证据，但 `bicycle` 和 `stump` 单场景增点分别为 +105,764 和 +144,738，说明下一步仍要做容量收益门槛、自适应 importance 或更轻量 descriptor 调用频率控制，而不能把 top-k25 max 直接作为预算效率结论。
