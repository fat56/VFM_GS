...
12. DINO descriptor top-k25 质量优先档已完成同四场景复验。它保持 `vfm_weight=0.0`，只把 descriptor metric map 从 top-k15 扩到 top-k25。四场景相对 matched FastGS densify100 平均提升 +0.1785 PSNR、+0.0078 SSIM、LPIPS 改善 -0.0141，平均 Gaussian 数量增加 78,143；四个场景三项指标全部正向。相对 top-k15，它继续平均提升 +0.0962 PSNR、+0.0020 SSIM、LPIPS 改善 -0.0032，但平均多 21,939 个点。`bicycle` 和 `stump` 的点数增量分别达到 +105,764 和 +144,738，因此 top-k25 应定位为质量优先档，不能直接作为预算效率结论。下一步优先跑 descriptor `weighted i0.50`，验证是否能在保留质量收益的同时压低高增点场景的容量增长。
13. DINO descriptor weighted i0.50 预算效率探测完成。它保持 top-k15、`vfm_weight=0.0`，把 importance 融合改为 `weighted + importance_weight=0.50`。四场景相对 matched FastGS densify100 平均只多 4,067 个 Gaussians，PSNR +0.0080、SSIM +0.0013、LPIPS 改善 -0.0025；`bicycle` 甚至少 13,288 个点。但 PSNR 在 `bicycle` 和 `stump` 轻微负向，说明该档位过于保守，适合作为接近 baseline 容量的低风险档，不适合作为主要质量贡献证据。下一步应跑 `top-k25 + weighted i0.50`，尝试结合 top-k25 的质量覆盖和 weighted 的容量控制。
