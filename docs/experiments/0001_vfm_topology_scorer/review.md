...
6. `weighted + importance_weight=0.50` 是当前近预算效率点。bicycle 表明它贴近 cadence 预算仍优于 cadence control，treehill 表明它能省点但不能解决 PSNR 负向，stump 表明它在大收益场景能省点并完整保住收益。下一步优先用 `counter` 或 `garden` 这类已知正向场景复验，观察 weighted 是否能在低点数增长和中等点数增长场景继续稳定。
