...
- MipNeRF360 全 9 场景 weighted quality pick 在每个场景按规则选择 i0.50 或 i0.75：bicycle、counter、garden、room、stump 选 i0.75，bonsai、flowers、kitchen、treehill 回退 i0.50。该选择均值为 PSNR 28.8636、SSIM 0.8665、LPIPS 0.1393、257,223 个 Gaussians、训练 151.14s。相比 weighted i0.50，三项质量均提升：+0.0130 PSNR、+0.0005 SSIM、LPIPS 改善 -0.0004，只多 2,487 个 Gaussians；相比普通 i0.50，PSNR 高 +0.0058，点数少 6,349。因此本轮的正向结论是“场景级档位选择”，不是固定 i0.75。
...
- 这一路径不引入在线 DINO inference，成本明显低于 descriptor 系列。当前 0001 在 bicycle 上的最佳质量结论仍定为 top-k 25% 完整对照；预算方向把 `weighted + importance_weight=0.50` 作为已完成全场景验证的效率候选，把 `max + importance_weight=0.50` 作为全场景质量 candidate。MipNeRF360 全场景复验支持 i0.50 的跨场景有效性，尤其 `counter` 在仅 +5.9% Gaussian 的条件下获得三项指标提升，`kitchen` 在少于 baseline 点数下仍获得三项指标提升，`room` 证明高基线小室内场景上也能略超 cached-edge v1，`stump` 则给出全场景中最大的 PSNR 增益；`treehill` 显示 DINO i0.50 能修复 cached-edge 负例的 SSIM/LPIPS，但预算膨胀明显且 PSNR 仍略低于 baseline。weighted 全场景复验显示：固定 i0.50 平均质量几乎贴住普通 i0.50，同时少 8,836 个 Gaussians、训练少 2.87s；固定 i0.75 不是全场景默认值，但按场景选择 i0.50/i0.75 后，平均 PSNR 超过普通 i0.50 和 weighted i0.50，平均点数只比 weighted i0.50 多 2,487。因此下一步主线应转向场景级档位选择，而不是继续寻找单一固定 importance weight。adaptive weighted 在 treehill 第二场景未通过，后续不升级为主线。
