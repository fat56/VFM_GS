...
主对照数据集：`datasets/mipnerf360/bicycle`，test split，`-r 8`。先用 620-step 验证链路健康，再跑 30,000 iterations 完整对照。后续用同一设置在 `garden`、`counter`、`treehill`、`bonsai`、`flowers`、`kitchen` 和 `room` 做跨场景复验，训练仍用 train split，指标来自 test split render。
...
- 八个代表场景目前形成比较清晰的形态：`bicycle` 用于和历史方法直接比较，`garden` 与 `flowers` 证明室外植被/高频纹理上 DINO i0.50 超过 edge proxy，`counter`、`bonsai`、`kitchen` 和 `room` 证明室内场景上也能超过 baseline 与 cached-edge v1，其中 `kitchen` 在低于 baseline 点数的条件下三项正向，`room` 相比 cached-edge v1 以更短训练时间取得三项微幅正向；`treehill` 说明 DINO 能修复 edge proxy 负例的 SSIM/LPIPS，但容量膨胀和 PSNR 轻微回落仍需要下一版预算机制处理。
- 八场景 candidate 均值为 PSNR 29.0136、SSIM 0.8728、LPIPS 0.1317、250,821 个 Gaussians、训练 140.80s。相比这八个场景的 baseline 均值，DINO i0.50 平均提升 +0.1763 PSNR、+0.0100 SSIM、LPIPS 改善 -0.0215，但平均多 77,157 个 Gaussians、训练多 14.88s；相比 cached-edge v1 均值，平均提升 +0.1081 PSNR、+0.0068 SSIM、LPIPS 改善 -0.0141，平均多 38,278 个 Gaussians、训练多 2.00s。
- 因此 `top-k 25% + importance_weight=0.50` 可以暂定为 0001 v1 candidate：它在 representative subset 上比 baseline 和 cached-edge v1 更强，但不是预算受控版本。下一步应补齐 MipNeRF360 其余 1 个场景，得到完整均值后再决定是否进入自动容量保护实现。
...
- 这一路径不引入在线 DINO inference，成本明显低于 descriptor 系列。当前 0001 在 bicycle 上的最佳质量结论仍定为 top-k 25% 完整对照；预算方向把 `importance_weight=0.50` 作为约 440k 点数下的效率点，把 `importance_weight=0.75` 作为略优于 top-k 15% 的高质量预算点。`garden`、`counter`、`bonsai`、`flowers`、`kitchen` 和 `room` 复验支持 i0.50 的跨场景有效性，尤其 `counter` 在仅 +5.9% Gaussian 的条件下获得三项指标提升，`kitchen` 在少于 baseline 点数下仍获得三项指标提升，`room` 证明高基线小室内场景上也能略超 cached-edge v1；`treehill` 则显示 DINO i0.50 能修复 cached-edge 负例的 SSIM/LPIPS，但预算膨胀明显且 PSNR 仍略低于 baseline。继续把权重推到 1.0 基本回到完整 top-k25 已知上界；support-normalized score 和高置信 prune-protect 都未优于普通 partial importance。下一步应优先做剩余 MipNeRF360 场景汇总和自动容量保护，而不是追加同类 scorer 单点。
