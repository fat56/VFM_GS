...
- 这一路径不引入在线 DINO inference，成本明显低于 descriptor 系列。当前 0001 在 bicycle 上的最佳质量结论仍定为 top-k 25% 完整对照；预算方向把 `weighted + importance_weight=0.50` 作为约 415k 点数下的新效率点，把 `max + importance_weight=0.50` 作为约 440k 点数下的全场景质量 candidate，把 `importance_weight=0.75` 作为略优于 top-k 15% 的高质量预算点。MipNeRF360 全场景复验支持 i0.50 的跨场景有效性，尤其 `counter` 在仅 +5.9% Gaussian 的条件下获得三项指标提升，`kitchen` 在少于 baseline 点数下仍获得三项指标提升，`room` 证明高基线小室内场景上也能略超 cached-edge v1，`stump` 则给出全场景中最大的 PSNR 增益；`treehill` 显示 DINO i0.50 能修复 cached-edge 负例的 SSIM/LPIPS，但预算膨胀明显且 PSNR 仍略低于 baseline。`treehill weighted` 进一步说明加权融合可以在压力场景中少点并基本保住质量，但不是完整预算解法。继续把权重推到 1.0 基本回到完整 top-k25 已知上界；support-normalized score 和高置信 prune-protect 都未优于普通 partial importance。420k、430k 线性和 430k quadratic 三个预算感知点说明软预算行为优于固定低权重的一部分指标，但还不能保住 i0.50 的主要质量收益；下一步在 `stump` 这种大收益/大点数场景复验 weighted，判断它能否保留主要增益同时明显降低点数。
