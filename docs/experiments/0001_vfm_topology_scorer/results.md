...
- 这一路径不引入在线 DINO inference，成本明显低于 descriptor 系列。当前 0001 在 bicycle 上的最佳质量结论仍定为 top-k 25% 完整对照；预算方向把 `weighted + importance_weight=0.50` 作为已完成全场景验证的效率候选，把 `max + importance_weight=0.50` 作为全场景质量 candidate，把 `importance_weight=0.75` 和 `weighted i0.75` 作为高质量档位。MipNeRF360 全场景复验支持 i0.50 的跨场景有效性，尤其 `counter` 在仅 +5.9% Gaussian 的条件下获得三项指标提升，`kitchen` 在少于 baseline 点数下仍获得三项指标提升，`room` 证明高基线小室内场景上也能略超 cached-edge v1，`stump` 则给出全场景中最大的 PSNR 增益；`treehill` 显示 DINO i0.50 能修复 cached-edge 负例的 SSIM/LPIPS，但预算膨胀明显且 PSNR 仍略低于 baseline。weighted 全场景复验显示：平均质量几乎贴住普通 i0.50，同时少 8,836 个 Gaussians、训练少 2.87s；`weighted i0.75` 在 bicycle、stump 和 room 上均比 weighted i0.50 有 PSNR 收益，其中 bicycle 三项指标同时正向，stump 三项指标微升但点数增加，room PSNR 正向且点数几乎不变但 LPIPS 极小幅回落。因此它应作为高质量档位，而不是默认预算效率档位。adaptive weighted 在 treehill 第二场景未通过，后续不升级为主线。
