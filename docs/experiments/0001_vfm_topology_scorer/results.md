...
主对照数据集：`datasets/mipnerf360/bicycle`，test split，`-r 8`。先用 620-step 验证链路健康，再跑 30,000 iterations 完整对照。后续用同一设置在 `garden`、`counter`、`treehill`、`bonsai` 和 `flowers` 做跨场景复验，训练仍用 train split，指标来自 test split render。
...
- 六个代表场景目前形成比较清晰的形态：`bicycle` 用于和历史方法直接比较，`garden` 与 `flowers` 证明室外植被/高频纹理上 DINO i0.50 超过 edge proxy，`counter` 和 `bonsai` 证明室内场景上也能超过 baseline 与 cached-edge v1，`treehill` 说明 DINO 能修复 edge proxy 负例的 SSIM/LPIPS，但容量膨胀和 PSNR 轻微回落仍需要下一版预算机制处理。
- 六场景 candidate 均值为 PSNR 27.6169、SSIM 0.8418、LPIPS 0.1603、290,233 个 Gaussians、训练 141.28s。相比这六个场景的 baseline 均值，DINO i0.50 平均提升 +0.1787 PSNR、+0.0125 SSIM、LPIPS 改善 -0.0274，但平均多 102,063 个 Gaussians、训练多 15.71s；相比 cached-edge v1 均值，平均提升 +0.1227 PSNR、+0.0090 SSIM、LPIPS 改善 -0.0186，平均多 49,790 个 Gaussians、训练多 2.38s。
- 因此 `top-k 25% + importance_weight=0.50` 可以暂定为 0001 v1 candidate：它在 representative subset 上比 baseline 和 cached-edge v1 更强，但不是预算受控版本。下一步应补齐 MipNeRF360 其余 3 个场景，得到完整均值后再决定是否进入自动容量保护实现。
