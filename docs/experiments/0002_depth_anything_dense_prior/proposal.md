...
- `configs/experiments/0002_depth_anything_depth_prior_prune_protect_auto_topk.yaml`

当前优先级已转为 direct relative depth prior，并在此基础上尝试 RGB-gated rerank 的保守扫描。depth-edge prior 只保留为弱混合信号对照，不继续扩展。2026-05-18 的 `l0.10 + broad035` 说明缩小 RGB broad candidate 能压低部分容量代价，但仍不能让 `stump/playroom/truck` 同时稳定正向，也不能解决 `truck` depth prior 与 RGB 瓶颈错位。`start9000` 后期介入能让 `playroom` 回到薄正向，但 `stump` 容量失控、`truck` 仍负向。按用户建议扩展到 MipNeRF360 全 9 场景后，均值质量小幅正向但平均多 237,716 个 Gaussians，9/9 场景 QCGI 为负；该结果不支持继续把当前 RGB-gated depth rerank 作为训练主线。2026-05-18 的 prune-protect 分支显示后期辅助裁剪比 densify rerank 稳定得多，但 `topk010`、`weight015`、`topk005` 三轮都没有形成跨场景稳定正收益：`topk010` 三场景平均接近持平，`weight015` 证明单独降保护权重会伤害 playroom，`topk005` 证明继续收窄 RGB proposal 会让 truck 变好但 playroom 变差。2026-05-19 将 `topk010` 扩到 MipNeRF360 全 9 场景后，均值为 27.9128 / 0.8196 / 0.2162、1,159,451 GS，相对 Phase 0 为 -0.0462 PSNR、-0.0007 SSIM、LPIPS +0.0005、GS -2,335，QCGI -0.0643。随后 train-side selector probe 只在 `kitchen` 选中 depth，其余 8/9 场景仍回到 baseline，混合均值只比 baseline 高约 +0.011 PSNR。2026-05-19 进一步补做 scene-adaptive `rgb_prune_auto_topk`：四景 pilot 比 fixed `topk010` 更接近 baseline，全 MipNeRF360 9 场景均值为 27.9723 / 0.8201 / 0.2162、1,162,093 GS，相对 Phase 0 为 +0.0133 PSNR、-0.0001 SSIM、LPIPS +0.0005、GS +307，QCGI +0.0065；但 DB/Tandt 四场景交叉验证只有 27.3632 / 0.8847 / 0.2068、593,032 GS，相对 Phase 0 为 -0.0011 PSNR、+0.0002 SSIM、LPIPS +0.0002、GS -328，QCGI +0.0007。随后补完 fixed `topk010` 在 DB/Tandt 的缺口后，它在完整 4 场景上为 27.2961 / 0.8849 / 0.2069、593,536 GS，相对 Phase 0 为 -0.0682 PSNR、+0.0004 SSIM、LPIPS +0.0002、GS +176，QCGI -0.0629。DB/Tandt 的 selector probe 进一步显示：DB 上 selector 只能把 `drjohnson` 选成 fixed `topk010`、`playroom` 选成 auto-topk，但 test 均值仍低于 baseline；Tandt 上 `train_qcgi` 只比 baseline 高一点点。当前判断是 Depth Anything prune-side auxiliary 有局部轻量修正价值，`auto-topk` 只是 MipNeRF360 上的弱正向候选，而 fixed `topk010` 在跨数据集上更差；下一步如果继续 0002，应转向正式的 validation-driven selector 或 online residual。
...
已完成的最小轮次是 `prune-protect-only`：FastGS/RGB 继续决定 densification，Depth Anything 只在 densification 结束后保护 RGB 高 pruning-score 候选。`stump/playroom/truck` 的三场景平均几乎持平，说明这条路比 RGB rerank 更像“轻量后验修正”，但还不是稳定默认解。`prune-protect weight015` 已验证单独降权重不够；`topk005` 已验证继续收窄 RGB proposal 也不够，三场景平均为 -0.0185 PSNR、-0.0003 SSIM、LPIPS +0.0010、GS -6,755，QCGI -0.0295。`topk010` 的 MipNeRF360 全 9 场景扩展进一步给出负向均值，说明固定 prune-protect 不应继续作为 0002 默认训练策略。下一步若继续 0002，应转向场景自适应门控、在线 depth residual 或 validation-driven selector，而不是继续手工扫相邻 top-k/weight。长任务继续使用 tmux/detached 方式运行；每轮完成后更新文档、commit 并 push，其中 validation-driven selector 需要独立 validation split，而不是这次 train probe。
