...
因此，Depth Anything prune-side auxiliary 的价值应被限定为“可能有局部后验修正信号”，而不是“可用固定规则”。继续固定 top-k/weight 扫描的边际价值已经很低。下一步若继续 0002，更合理的是做场景自适应 protect、在线 depth residual，或者利用 validation/train-side 短评估决定是否启用 protect。2026-05-19 的 train-side selector probe 已经能在 baseline / auto-topk / topk010_full 之间切换，但 `train_best_psnr` 和 `train_qcgi` 的 test 均值都略低于 baseline，说明它更像保守路由器，不是默认增益策略。

## 2026-05-19 prune-protect train selector probe

这轮 selector 先修正了输入名里 `depth_anything_depth_prior_prune_protect_topk010_full` 的 `_full` 后缀不匹配，才把 27 条 input rows 全部跑完。`train_best_psnr` 会在 baseline、auto-topk、topk010_full 三者间切换：baseline 2 场景，auto-topk 4 场景，topk010_full 3 场景；`train_qcgi` 更保守：baseline 3 场景，auto-topk 5 场景，topk010_full 1 场景。
